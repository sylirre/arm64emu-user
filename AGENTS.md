...
src/
  ...
  proctab.c                          Shared-memory guest-PID registry (cross-process ps/top view, plus the id maps of a faked user namespace, which a parent writes for its child, plus each process's seccomp state, which anyone reading its status needs) + unified IPC broker daemon backing System V IPC: owns shm memfd/file backings (handed out over SCM_RIGHTS) and all semaphore/message-queue state, parks blocking semop/msgsnd/msgrcv waiters, applies SEM_UNDO at process death, self-cleans on idle
