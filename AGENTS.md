...
an emulator bug rather than the tier's slowness (see docs/jit.md). The rows
qemu-arm itself cannot run — it asserts on `mremap(old_size=0)` (two rows: the
emulator's own use of it, and the guest's), writes nothing for
`getsockopt(SO_RCVTIMEO)`, and ignores `waitid`'s rusage argument — carry a
`NEEDS-HOST-SYSCALL:` marker (in a C test or a fixture alike) and skip there,
naming what is missing; they run in full on real armv7 silicon.
...
src/
  ...
  sys_proc.c                         Process syscalls (fork/exec/wait/kill, CLONE_VM threads; vfork as a fork whose parent waits and whose child's writes are carried back); the guest thread registry (every live thread's robust-list head and personality); execve's cooperative de_thread (rendezvous siblings at a safepoint, land the new image on the main thread); a main thread that exit(2)s while siblings run parks as the kernel's zombie leader instead of ending the process
  ...
  sys_procfs.c                       Synthesized guest /proc (maps, cmdline, mounts, stat, writable uid_map/gid_map/setgroups of a faked user namespace -- its own or, the usual arrangement, a child's, per-process status rebuilt line by line so TracerPid/Seccomp/Sig*/Cap* describe the guest and not the emulator, every task's personality, ...)
  ...
  proctab.c                          Shared-memory guest-PID registry (cross-process ps/top view, plus the id maps of a faked user namespace, which a parent writes for its child, plus each process's seccomp state, which anyone reading its status needs, plus the host tasks in its thread group that are not guest threads -- an interposer's own -- so the guest is never shown one, plus its main thread's and default personality) + unified IPC broker daemon backing System V IPC: owns shm memfd/file backings (handed out over SCM_RIGHTS) and all semaphore/message-queue state, parks blocking semop/msgsnd/msgrcv waiters, applies SEM_UNDO at process death, holds any thread personality that differs from its process's, self-cleans on idle
...
* `A64_PINWALK_FORCE_LOOP`: pins a resolved path's parent one component at a time (openat/O_NOFOLLOW) instead of in a single `openat2(RESOLVE_NO_SYMLINKS)` — the tier a host kernel older than 5.6 is served by, and the one a seccomp filter over openat2 puts a modern host on. Same containment, one syscall per component instead of one per path — and two descriptors open at once instead of one, which is why the guest's `RLIMIT_NOFILE` is judged from the walk's first allocation and not its last (`tests/c/fdlimit.c`, whose `(loop-tier)` row runs over this).
...
* `A64_PROCTAB_FORCE_FILE`: skips the diskless guest-PID registry broker (memfd over an abstract socket) for the named-file tier — what a host with neither memfd_create nor abstract sockets is permanently on. The file's name is fixed and its directory is world-writable, so it is opened `O_NOFOLLOW` and refused unless it is a regular file of this user with no other link and no group/other access; a refusal degrades to the per-invocation anonymous table and says so on stderr.
* `A64_SHM_FORCE_FILE`: forces System V shm segments onto file backing (an unlinked `mkstemp` file in the first writable dir) instead of an anonymous memfd, exercising the fallback tier.
...
* `A64_GETRANDOM_FORCE_OLD`: refuses `GRND_INSECURE` on the host getrandom(2) path the way a 3.17–5.5 kernel does (every Android 10/11 kernel) — the tier such a host is served by, where the flag the guest's 6.1 ABI promises is answered from /dev/urandom instead of with the host's EINVAL; the suite's `c/getrandom(old-getrandom tier)` row runs over it.
...
* `A64_MINCORE_FORCE_FAIL`: makes the resident-set sample fail — the tier a host that will not answer mincore(2) (a sandbox, a seccomp policy) is served by, where what a guest reads about its own memory keeps the guest's sizes and falls back to the host's Rss figures, bounded by what the guest mapped. That is the same tier every reader of *another* process is permanently on; the suite's `vmreport (no-mincore tier)` row runs over it.
...
* `A64_MEMFD_SEAL_FORCE_OLD`: refuses a read-only shared mapping of a write-sealed memfd the way a kernel older than 6.x does -- the tier every current LTS kernel puts a host on, where the deny-writable reference the seal takes is counted against every shared mapping before the kernel asks whether that mapping could write at all. The emulator serves the guest the 6.x mapping from backing of its own (the file cannot change under that seal, so nothing about it is guest-visible), and the suite's `(old-seal-mmap tier)` row runs over it.
...
* `A64_OWNFD_FORCE_DENY`: refuses the path spelling of the emulator's own fds (`/proc/self/fd/N`) — the tier Android's SELinux policy serves, where a memfd cannot be re-opened through that link, so exec and open must reach the image through the descriptor instead; harsher than the device (stat and access are refused too), and the suite's `(path-denied tier)` rows run over it.
...
* `XDG_RUNTIME_DIR`, `TMPDIR`: first writable one holds the backing directories of emulated tmpfs mounts (removed when the session ends; a session killed before it could clean up is swept by the next one), plus the --shared-proc *fallback* registry file and the System V shm segment backings when the diskless broker (memfd) is unavailable and /dev/shm isn't writable (Termux). The shm backings are unlinked the moment they are created (the broker hands attachers the descriptor, never a name), so only the registry file has one — and it is refused unless it is a regular file owned by this user that nobody else can reach.
