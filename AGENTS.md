...
* `XDG_RUNTIME_DIR`, `TMPDIR`, `PREFIX`: first writable one holds the --shared-proc *fallback* registry file, used only when the diskless broker (memfd) is unavailable and /dev/shm isn't writable (Termux).
