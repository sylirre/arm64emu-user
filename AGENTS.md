...
src/
  ...
  sys_procfs.c                       Synthesized guest /proc (maps, cmdline, mounts, stat, writable uid_map/gid_map/setgroups of a faked user namespace -- its own or, the usual arrangement, a child's, ...)
  ...
  proctab.c                          Shared-memory guest-PID registry (cross-process ps/top view, plus the id maps of a faked user namespace, which a parent writes for its child) + unified IPC broker daemon backing System V IPC: owns shm memfd/file backings (handed out over SCM_RIGHTS) and all semaphore/message-queue state, parks blocking semop/msgsnd/msgrcv waiters, applies SEM_UNDO at process death, self-cleans on idle
