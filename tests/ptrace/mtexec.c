 ...
 *
 * Three things have to reach the tracer that nothing else generates. Every
 ...
 * exits; the exec stop has to arrive on the *main* thread's tid even though a
 * different thread asked for the exec -- a kernel hands that thread the
 * leader's pid, and the emulator lands the new image on the main thread,
 * which already has it -- with PTRACE_GETEVENTMSG naming the tid the exec'ing
 * thread had; and that old tid is simply gone: it is not a thread that died,
 * so no exit is reported for it (ptrace(2), "execve(2) under ptrace").
 *
 ...
 * The expectations are a kernel's: built natively for the host, this program
 * passes there too.
 *
...
int main(int argc, char **argv) {
    ...
    unsigned long tids[NTHR] = { 0 }, msg = 0;
    pid_t xtid;                             /* the thread that execs: made last */
    ...
    for (;;) {
        ...
        if ((status >> 8) == (SIGTRAP | (PTRACE_EVENT_CLONE << 8))) {
            if (nclones < NTHR) ptrace(PTRACE_GETEVENTMSG, w, 0, &tids[nclones]);
            nclones++;
        }
        int is_exec = (status >> 8) == (SIGTRAP | (PTRACE_EVENT_EXEC << 8));
        if (is_exec && w != a)
            return fail("exec stop did not arrive on the main tid");
        if (is_exec) ptrace(PTRACE_GETEVENTMSG, w, 0, &msg);
        ptrace(PTRACE_CONT, w, 0, 0);
        if (is_exec) break;
    }
    if (nclones != NTHR) return fail("not every thread creation was reported");
    xtid = (pid_t)tids[NTHR - 1];
    if ((pid_t)msg != xtid) return fail("GETEVENTMSG is not the exec'ing thread's tid");
    ...
    for (int ms = 0; ms < DRAIN_MS && ndeaths < NSPIN; ms += 5) {
        ...
        if (WIFEXITED(status)) {
            ...
            if (w == xtid) return fail("the exec'ing thread's old tid was reported");
            ndeaths++;
        } else {
            ptrace(PTRACE_CONT, w, 0, 0);
        }
